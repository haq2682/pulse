...
def create_campaign_roi_features(campaigns_df, orders_df, historical_stats_df, MIN_CAMPAIGN_DAYS):
    """
...
def prepare_training_data(df, target_column, MIN_RECORDS_THRESHOLD):
...
def main(BUCKET_NAME):
    ...
    campaign_valid, _, _ = validate_columns(campaigns_df, REQUIRED_CAMPAIGN_COLUMNS, "Campaigns", MAX_NULL_PERCENTAGE)
    ...
    print("-" * 60)
    df_features = create_campaign_roi_features(campaigns_df, orders_df, historical_stats, MIN_CAMPAIGN_DAYS)
    ...
    result_roi = prepare_training_data(df_features, TARGET_COLUMN_ROI, MIN_RECORDS_THRESHOLD)
    ...
    result_revenue = prepare_training_data(df_features, TARGET_COLUMN_REVENUE, MIN_RECORDS_THRESHOLD)
    ...
    save_model(model_revenue, "campaign_revenue", MODEL_OUTPUT_PATH)
