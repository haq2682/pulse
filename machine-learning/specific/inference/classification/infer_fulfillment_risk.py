...
def generate_simulated_features(df):
    """
    Simulated features are generated for display/output columns only.
    They are NOT in the feature lists and are not passed to the ML model.
    """
...
def load_model_and_preprocessors(spark, model_dir, model_name):
    ...
    GBT is saved as OneVsRestModel; all others use their native model class.
    """
    try:
        ...
        if model_name == "LogisticRegression":
            ...
            # GBT wrapped in OneVsRest during training → load as OneVsRestModel
            model = OneVsRestModel.load(model_path)
        else:
            raise ValueError(f"Unknown model name: {model_name}")

        categorical_indexers = []
        for i in range(len(CATEGORICAL_FEATURES)):
            indexer = StringIndexerModel.load(f"{model_dir}/{model_name}_cat_indexer_{i}")
            categorical_indexers.append(indexer)

        scaler = StandardScalerModel.load(f"{model_dir}/{model_name}_scaler")
...
def prepare_features(df, preprocessors):
    ...
    df_filled = preprocessors["scaler"].transform(df_filled)
...
def generate_predictions(spark, df, model, model_name, MODEL_VERSION):
    """
    Generate comprehensive predictions.
    OneVsRest (GBT) does not produce a probability column, so we handle both
    probability-bearing models and prediction-only models gracefully.
    """
    ...
    calc_ship_date = udf(
        lambda placed_at, risk: (
            datetime.fromisoformat(str(placed_at)) + timedelta(days=[1, 2, 3, 5][int(risk)])
        ).date().isoformat() if placed_at else None,
        StringType()
    )
    calc_delivery_date = udf(
        lambda placed_at, risk: (
            datetime.fromisoformat(str(placed_at)) + timedelta(days=[5, 8, 12, 20][int(risk)])
        ).date().isoformat() if placed_at else None,
        StringType()
    )
    ...
    calc_delay_cost    = udf(lambda risk, amt: float(amt * [0.0, 0.05, 0.10, 0.20][int(risk)]) if amt else 0.0, DoubleType())
    calc_expedite_cost = udf(lambda risk, amt: float(amt * [0.0, 0.10, 0.15, 0.25][int(risk)]) if amt else 0.0, DoubleType())
    calc_compensation  = udf(lambda risk, amt: float(amt * [0.0, 0.05, 0.10, 0.25][int(risk)]) if amt else 0.0, DoubleType())

    # Build output — probability columns fall back to NaN-safe defaults for
    # OneVsRest / GBT which lacks a probability column.
    base_cols = predictions.select(
    ...
    if HAS_PROBABILITY:
        ...
        # GBT / OneVsRest: no probability column — fill prob fields with -1.0
        output_df = base_cols \
            .withColumn("prob_low_risk", lit(-1.0)) \
            .withColumn("prob_medium_risk", lit(-1.0)) \
            .withColumn("prob_high_risk", lit(-1.0)) \
            .withColumn("prob_critical_risk", lit(-1.0)) \
            .withColumn("delay_probability", lit(-1.0)) \
            .withColumn("failure_probability", lit(-1.0)) \
            .withColumn("partial_fulfillment_probability", lit(-1.0)) \
            .withColumn("delivery_window_confidence", lit(-1.0)) \
            .withColumn("prediction_confidence", lit(-1.0))

    generate_uuid = udf(lambda: str(uuid.uuid4()), StringType())
    output_df = output_df.withColumn("prediction_id", generate_uuid())

    count_result = output_df.count()
    print(f"✓ Generated {count_result} predictions")
    return output_df
