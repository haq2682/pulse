...
def create_ml_spark_session(
    ...
    if not is_local:
        builder = (
            ...
            # has no NetworkPolicy rule that can allow a port that changes
            # every run - verified live in cleaning/cleaning_config.py's
            # identical fix (approxQuantile hung for 30 minutes on exactly
            # this). ML is disabled project-wide - not run/tested, kept
            # consistent for when it's re-enabled.
