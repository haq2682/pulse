...
def transform_customers(dataframes):
    customers_with_orders = (
        ...
        .join(
            ...
            .groupBy("customer_id")
            .agg(
                datediff(current_date(), max("order_placed_at")).alias(
                    "order_recency_days"
                ),
...
def add_customer_aggregations(dataframes):
    # FIX: Aggregate order_items first to get quantity per order
    order_items_agg = (
    ...
    # Join orders with aggregated order_items (not the raw table)
    orders_with_items = (
        dataframes["orders"]
    ...
    customer_order_agg = orders_with_lag.groupBy("customer_id").agg(
        ...
        min(when(col("order_placed_at").isNotNull(), col("order_placed_at"))).alias(
            "first_order_date"
        ),
        max(when(col("order_placed_at").isNotNull(), col("order_placed_at"))).alias(
            "last_order_date"
        ),
        avg(
            when(
    ...
    customer_session_agg = (
        ...
        .agg(
            ...
            (
                sum(when(col("conversion_flag") == 1, lit(1)).otherwise(lit(0)))
                / count("session_id")
            ).alias("session_conversion_rate"),
            (
                sum(when(col("cart_abandonment_flag") == 1, lit(1)).otherwise(lit(0)))
                / count("session_id")
    ...
    customer_wishlist_agg = (
        ...
        .agg(
            ...
            (
                sum(when(col("purchased_date").isNotNull(), lit(1)).otherwise(lit(0)))
                / count("wishlist_id")
            ).alias("wishlist_conversion_rate"),
        )
    )

    # FIX: Deduplicate payment methods per customer first
    customer_payment_agg = (
        dataframes["payments"]
        .join(
            dataframes["orders"].select("order_id", "customer_id").distinct(),
            "order_id",
            "inner",
        )
        .filter(col("customer_id").isNotNull() & col("payment_method").isNotNull())
        .dropDuplicates(["customer_id", "payment_method"])
        .groupBy("customer_id")
        .agg(expr("first(payment_method)").alias("preferred_payment_method"))
    )

    # Join all aggregations and add final deduplication
    dataframes["customers"] = (
        dataframes["customers"]
