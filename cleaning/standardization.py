...
from pyspark.sql.functions import current_date, current_timestamp, when, col, length, trim
from pyspark.sql.types import DateType, TimestampType, IntegerType, LongType, FloatType, DoubleType, DecimalType
...
def detect_gibberish_patterns(dataframes):
    ...
    status_validations = {
        ...
        "orders": {"order_status": ["Pending", "Processing", "Shipped", "Delivered", "Cancelled", "Returned"]},
        "payments": {"payment_status": ["Pending", "Completed", "Failed", "Refunded", "Partially Refunded"]},
    ...
    if "orders" in dataframes:
        ...
        if "currency" in df.columns:
            valid_currencies = ["USD", "EUR", "GBP", "CAD", "AUD", "JPY", "INR", "Unknown"]
